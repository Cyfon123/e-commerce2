...
import { Box, Grid, TextField, Button, MenuItem, FormControl, InputLabel, Select } from '@mui/material';
...
const AddressForm = () => {
  ...
  const [states] = useState(state_arr);
  ...
  const handleStateChange = (event) => {
    ...
    setSelectedState(selectedStateIndex);
    

    setFormData((prevData) => ({ ...prevData, state: selectedStateIndex, city: '' }));

    // Update cities based on selected state
    if (selectedStateIndex) {
      const cityList = getCitiesForState(selectedStateIndex);
      setCities(cityList);
    } else {
      setCities([]);
    }
  ...
  // Hardcoded function to simulate fetching cities based on the state
  const getCitiesForState = (stateIndex) => {
    // Check if the stateIndex is valid and return corresponding cities
    const cityString = s_a[stateIndex];
    return cityString ? cityString.split(" | ") : [];
  };
  ...
  const handleSubmit= async (e)=>{
    ...
    const address = {
        ...
        city: formData.city,
        state: state_arr[formData.state],
